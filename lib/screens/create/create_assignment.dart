...
class _CreateAssignmentState extends State<CreateAssignment> {
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: <Widget> [
                          ...
                          const SizedBox(height: 20),
                          ElevatedButton(onPressed: addAssignment, child: const Text("Add Assignment")),
  ...
  addAssignment() async {
    ...
    try {
      FirebaseFirestore.instance.collection("tasks").add(Task(user.uid, _selectedModule.trim(), "Assignment", nameController.text.trim(), descriptionController.text.trim(), startDate, endDate, repeatDaily, repeatWeekly, repeatMonthly, repeatYearly, allDay).toJson());
      Utils.showSnackBar("Created assignment ${nameController.text.trim()}", false);
      showDialog(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog(
              content: const Text("Would you like to add another assignment?"),
              actions: [
                TextButton(
                    onPressed: () {
                      setState(() {
