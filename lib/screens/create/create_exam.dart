...
class _CreateExamState extends State<CreateExam> {
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: <Widget> [
                          ...
                          const SizedBox(height: 20),
                          ElevatedButton(onPressed: addTutorial, child: const Text("Add Exam")),
  ...
  addTutorial() async {
    ...
    try {
      FirebaseFirestore.instance.collection("tasks").add(Task(user.uid, _selectedModule.trim(), "Exam", nameController.text.trim(), descriptionController.text.trim(), startDate, endDate, repeatDaily, repeatWeekly, repeatMonthly, repeatYearly, allDay).toJson());
      Utils.showSnackBar("Created exam ${nameController.text.trim()}", false);
      showDialog(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog(
              content: const Text("Would you like to add another exam?"),
              actions: [
                TextButton(
                    onPressed: () {
                      setState(() {
